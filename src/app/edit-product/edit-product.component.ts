					...
					img.onload = () => {
						...
						if (height != width) {
							...
							if(this.urls.length < 6) {
								if(this.urls.find(item=>item.name  == name) == undefined) {
									...
									this.commonService.errorToast('Duplicate image detected ')
								}
							} else {
								this.commonService.errorToast('Maximum 6 images can be uploaded')
							}
